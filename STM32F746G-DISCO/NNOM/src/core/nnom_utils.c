...
volatile uint64_t Time_cnt1, Time_cnt2=0;
...
nnom_predict_t *prediction_create(nnom_model_t *m, int8_t *buf_prediction, size_t label_num, size_t top_k_size)
{
	...
	// mark start time.
	char uart_buf[50];
	int uart_buf_len1,uart_buf_len2=0;

	Time_cnt1 = __HAL_TIM_GET_COUNTER(&htim10);		////lsr 23.07.07
	pre->t_predict_start = Time_cnt1;
...
void prediction_end(nnom_predict_t *pre)
{
	if (!pre)
		return;
	Time_cnt2 = __HAL_TIM_GET_COUNTER(&htim10) - Time_cnt1;
	pre->t_predict_total = Time_cnt2;
}
...
void NNOM_LOG(const char* format, ...)
{
    ...
    va_end(args);
    HAL_UART_Transmit(&huart6, (uint64_t*)buf, len, HAL_MAX_DELAY);
}
int nnom_us_get(void)
{
	//timer_val = __HAL_TIM_GET_COUNTER(&htim16);
	int ret = __HAL_TIM_GET_COUNTER(&htim10);
	//int ret = htim16.Instance->CNT;
	return ret;
}
int nnom_ms_get(void)
{
