...
volatile uint64_t OPS_data;
volatile uint16_t uart_buf5[20];
volatile uint16_t uart_buf6[20];
...
static uint64_t model_set_ops(nnom_model_t *m)
{
	...
	//uart_buf_len3 = sprintf(uart_buf3, "total ops : %d \r\n", OPS_data);
	//HAL_UART_Transmit(&huart6, (uint8_t *)uart_buf3, uart_buf_len3, 100);
...
nnom_status_t layer_run(nnom_layer_t *layer)
{
	...
	// done
	layer->stat.time = ((1000*nnom_ms_get())+nnom_us_get()) - start;
	return result;
}
