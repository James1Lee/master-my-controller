...
extern uint64_t TimerCountmSec;

void PrintTimeStamp(void)
{
	NNOM_LOG("this is time stamp test function - %ld \r\n", TimerCount);
}
...
nnom_predict_t *prediction_create(nnom_model_t *m, int8_t *buf_prediction, size_t label_num, size_t top_k_size)
{
	...
	// mark start time.
	pre->t_predict_start = (1000*nnom_ms_get())+nnom_us_get();
...
nnom_status_t prediction_run(nnom_predict_t *pre, uint32_t true_label, uint32_t*predict_label, float* prob)
{
	...
	uint32_t start;
	uint32_t sum = 0;
	uint32_t timer_val4, timer_val5;
	char uart_buf4[50];
	int uart_buf_len4;
	...
	//TimerCount = 0;	////lsr
	//timer_val4 = TimerCount;
	//HAL_GPIO_WritePin(GPIOI, GPIO_PIN_3, GPIO_PIN_SET);
	start = (1000*nnom_ms_get())+nnom_us_get();
	model_run(pre->model);
	pre->t_run_total += ((1000*nnom_ms_get())+nnom_us_get()) - start;

	//timer_val5=TimerCount;
	//uart_buf_len4 = sprintf(uart_buf4, "%u ms\r\n", (timer_val5-timer_val4));
	//HAL_UART_Transmit(&huart6, (uint8_t *)uart_buf4, uart_buf_len4, 100);
...
void prediction_end(nnom_predict_t *pre)
{
	if (!pre)
		return;
	pre->t_predict_total = ((1000*nnom_ms_get())+nnom_us_get()) - pre->t_predict_start;
...
void NNOM_LOG(const char* format, ...)
{
    char buf[256];
    va_list args;
...
uint32_t nnom_us_get(void)
{
	uint32_t ret = htim7.Instance->CNT;
	return ret;
}
uint32_t nnom_ms_get(void)
{

	//return (nnom_us_get()/1000);
	return TimerCount;
}
...
void prediction_summary(nnom_predict_t *pre)
{
	...
	NNOM_LOG("Test running time: %ld sec\n", pre->t_predict_total / 1000);
	NNOM_LOG("Model running time: %ld ms\n", pre->t_run_total);
	if(pre->predict_count !=0)
		NNOM_LOG("Average prediction time: %ld us\n", (pre->t_run_total * 1000) / pre->predict_count);
...
void model_stat(nnom_model_t *m)
{
	...
	while (layer)
	{
		...
		uart_buf_len6 = sprintf(uart_buf6, "!!!! total ops : %d   layer_time : %d \r\n", total_ops, total_time);
		HAL_UART_Transmit(&huart6, (uint8_t *)uart_buf6, uart_buf_len6, 100);
		HAL_Delay(50);
	...
	NNOM_LOG("Prediction time :%ld us\n", (uint32_t)total_time);
	if(total_time != 0)
